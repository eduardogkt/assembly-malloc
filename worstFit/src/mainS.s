.section .data
    ...
    STR_BUFFER_INIT: .string "*************** allocator worst-fit assembly ***************\n"
    STR_HEADER:      .string "################"
    STR_PRINT:       .string "%s"
    STR_FILLED:      .string "+"
    STR_FREED:       .string "-"
...
#   -32(%rbp) = sizeBiggest
#   -40(%rbp) = ptrBiggest
#   retorn    = blockStart (start of the allocated block)
asMalloc_allocMem:
    pushq %rbp
    movq %rsp, %rbp
    pushq %r12       # saving %12, used by the callee
    subq $40, %rsp   # alcoando VLs
...
# scroll through the entire list to find the largest space
...
# if_alt1: (size > sizeBiggest)
    movq -24(%rbp), %rbx  # %rbx = size
    movq -32(%rbp), %rcx  # %rbx = sizeBiggest
    cmpq %rcx, %rbx
    jle end_if_alt1
...
# verifying if found a block of maximum size
# if_alt2: (ptrBiggest != NULL)
    movq -40(%rbp), %rbx
    cmpq $0, %rbx
    je end_if_alt2

    # nextSize = (sizeBiggest - numBytes - 16)
    movq -32(%rbp), %rbx  # %rbx = sizeBiggest
    movq %rdi, %rcx       # %rcx = numBytes
    subq %rcx, %rbx       # %rcx = sizeBiggest - numBytes
    subq $16, %rbx        # %rbx = sizeBiggest - numBytes - 16

    # fillBlock(ptrBiggest, FILLED, numBytes);
    pushq %rdi            # saving %rdi
    movq %rdi, %rdx       # param3: numBytes
    movq $FILLED, %rsi    # prarm2: state FILLED
    movq -40(%rbp), %rdi  # param1: ptrBiggest 
...
# if_2: (nextSize >= 1)
    ...
    movq -40(%rbp), %rcx  # %rcx = ptrBiggest
    addq $16, %rcx        # %rcx = ptrBiggest + 16
    addq %rdi, %rcx       # %rcx = ptrBiggest + 16 + numBytes

    # fillBlock(prox, FREED, nextSize);
    pushq %rdi         # salvando %rdi
    movq %rbx, %rdx    # param3: nextSize
    movq $FREED, %rsi  # prarm2: state FREED
    movq %rcx, %rdi    # param1: prox 
    call fillBlock     # %rax contains the start of the allocated block
    popq %rdi          # restoring %rdi
...
else_2:
    # *(ptr + 8) = sizeBiggest
    movq -40(%rbp), %rbx   # %rbx = ptrBiggest
    movq -32(%rbp), %rcx   # %rcx = sizeBiggest
    movq %rcx, 8(%rbx)     # *(ptrBiggest + 8) = sizeBiggest
...
# allocating a new node at the end of the list (same as the first fit approach)
end_if_alt2:
